...
export default function Appointments() {
  ...
  useEffect(() => {
    if (ttsEnabled && !hasSpoken.current) {
      ...
      speak("Pantalla de nueva cita. Seleccione una especialidad.");
    }
    return () => {
  ...
  useEffect(() => {
    ...
    if (step === "datetime") speak("Seleccione la fecha y hora de su cita.");
    if (step === "doctor") speak("Seleccione su médico.");
    if (step === "summary")
      speak("Resumen de su cita. Elija el método de pago.");
    if (step === "done")
      speak("Reserva confirmada. Su cita ha sido programada.");
    return () => {
      window.speechSynthesis.cancel();
    };
  }, [step, ttsEnabled]);

  const stepNum = { specialty: 1, datetime: 2, doctor: 2, summary: 3, done: 3 }[
    step
  ];
    ...
    <div className="step-indicator">
      {[1, 2, 3].map((n, i) => (
        <span key={n} style={{ display: "contents" }}>
          <div
            className={`step-circle ${stepNum > n ? "done" : stepNum === n ? "active" : ""}`}
          >
            {n}
          </div>
          {i < 2 && (
            <div className={`step-line ${stepNum > n ? "done" : ""}`} />
          )}
        </span>
      ))}
    </div>
  ...
  const renderSpecialty = () => (
    <div className="appts-page-wrapper">
      <StepIndicator />
      <p className="section-title ion-justify-content-center">
    ...
    <div className="appts-page-wrapper">
      ...
      <div className="calendar-card">
        ...
        <div className="cal-grid">
          {DAY_HEADERS.map((h) => (
            <div key={h} className="cal-day-header">
              {h}
            </div>
          ))}
          {cells.map((d, i) => (
  ...
  const renderSummary = () => (
    <div className="appts-page-wrapper">
      ...
      <div className="summary-card">
        ...
        <div className="summary-row">
          ...
          <p className="summary-row-value">
            {formattedTime} - {pad(timeHour + 1)}:00{" "}
            {timeHour + 1 < 12 ? "AM" : "PM"}
          </p>
        </div>
      ...
      <div className="payment-toggle">
        <button
          className={`payment-toggle-btn ${payMethod === "card" ? "active" : ""}`}
          onClick={() => setPayMethod("card")}
        >
          Con tarjeta
        </button>
        <button
          className={`payment-toggle-btn ${payMethod === "cash" ? "active" : ""}`}
      ...
      {payMethod === "card" ? (
        <>
          ...
          <p className="summary-total" style={{ marginBottom: "12px" }}>
            Total a Pagar: S/ 120.0
          </p>
        ...
        <>
          <div className="sin-tarjeta-box">
            <p className="sin-tarjeta-title">
              Desde la aplicación de un Banco (BCP, BBVA, Interbank)
            </p>
            <ul>
              ...
              <li>
                Ingresar el código generado y se mostrará el detalle de la
                reserva.
              </li>
          ...
          <div className="sin-tarjeta-box">
            <p className="sin-tarjeta-title">
              Pago presencial en el Banco (BCP, BBVA, Interbank)
            </p>
            <ul>
  ...
  const renderDone = () => (
    <div
      className="appts-page-wrapper"
      style={{ textAlign: "center", paddingTop: "48px" }}
    >
      <div style={{ fontSize: "56px", marginBottom: "16px" }}>✅</div>
      <h2 style={{ color: "#1c1c1e", fontWeight: 700, marginBottom: "8px" }}>
        ¡Reserva confirmada!
      </h2>
      <p style={{ color: "#636366", fontSize: "14px", marginBottom: "32px" }}>
        Tu cita ha sido programada. Puedes verla en Inicio.
      </p>
      ...
      {showCode && (
        <div className="confirm-modal-overlay">
          <div className="confirm-modal-box">
            ...
            <p>
              El detalle de la reserva fue enviado a su correo y tiene 2 horas
              para confirmar el pago, gracias.
            </p>
            <button
